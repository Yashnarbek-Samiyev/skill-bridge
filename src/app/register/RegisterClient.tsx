...
export default function RegisterClient({ dict }: { dict: Dictionary }) {
  const [state, action, isPending] = useActionState(handleRegisterAction, null)
      ...
      <div className="card animate-fade-in" style={{
        ...
        <form action={action} style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
          ...
          <button 
            ...
            disabled={isPending}
            style={{ width: '100%', padding: '16px', fontSize: '16px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.5px', opacity: isPending ? 0.7 : 1 }}
          >
            {isPending ? '...' : dict.register.btn}
          </button>
        </form>
